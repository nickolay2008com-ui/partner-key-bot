...
WELCOME_TEXT = """
💞 Астробот «Инструкция к вашему мужчине»

К бытовой технике инструкция есть. К человеку, которого вы любите, — обычно нет. Поэтому бот собирает мягкую карту понимания по дате рождения: где мужчине спокойно, как он слышит слова, что может восприниматься как нежность и какой шаг помогает сделать контакт яснее.

Что можно сделать за 2 минуты:
• ввести дату рождения мужчины;
• получить бесплатный первый ключ по его Луне;
• добавить свою дату и увидеть эмоциональный мост пары;
• выбрать один практичный шаг для переписки, разговора или встречи.

🤍 Это не приговор, не «проценты совместимости» и не повод терпеть плохое отношение.
🧭 Это навигация: меньше гадания по молчанию, больше ясности, тепла и действий, которые можно проверить в реальной жизни.
""".strip()

ABOUT_TEXT = """
Что делает бот:

1. Берёт дату рождения мужчины и, если хотите, вашу дату.
2. Считает Луну, Венеру, Меркурий, Марс и Юпитер через Swiss Ephemeris.
3. Сначала даёт бесплатный ключ: его эмоциональный язык, зону безопасности и один бережный шаг.
4. После добавления ваших данных показывает мост пары: где ритмы совпадают, где легко задеть друг друга и как начать разговор спокойнее.
5. Premium открывает подробные планеты, портреты пары и сообщение с эффектом — не ради «больше текста», а ради понятного сценария действия.

Формат — как инструкция к человеку: с лёгкой улыбкой, но без мистического давления. Каждый вывод переводится в наблюдаемый сигнал: что попробовать, на что смотреть и когда остановиться.

Важно: бот не заменяет терапию, безопасность и прямой разговор. Если в отношениях есть страх, угрозы, контроль или насилие — главный шаг не астрология, а поддержка и защита себя.
""".strip()
...
def premium_paywall_text(product_key: str) -> str:
    if product_key in PLANET_PAYWALL_COPY:
        ...
        return f"""
{planet_label}: один конкретный вопрос пары

Выбирайте этот блок, если сейчас важна одна тема, а не вся карта. За 50 ₽ вы получите его разбор, а {free_label} добавим для равноправного сравнения.

Что получите:
• его вероятный сценарий в этой теме;
• ваш сценарий — для сравнения двух ритмов;
• практичный мост для переписки, разговора или встречи;
• короткий шаг на сегодня без мистических обещаний.
...
""".strip()
    if product_key == "message":
        return """
✍️ 2 варианта сообщения по его эмоциональному ритму

Выбирайте этот блок, если хотите написать первой, вернуть разговор после паузы или предложить встречу, но боитесь сказать слишком много, резко или туманно.

Что получите:
• два черновика сообщения по стихии его Луны;
• ориентир по Меркурию — какой формат разговора ему может быть понятнее;
• подсказку по Венере — как добавить тепла и ценности;
• ориентир по Марсу — какой следующий шаг не усиливает напряжение;
• примеры формулировок, которых сейчас лучше избегать.

Это не волшебные фразы и не гарантия ответа. Выберите подходящий черновик и перепишите его своими словами, сохранив собственный смысл и границы.

Разбор останется в этой карте пары.

Стоимость: 149 ₽
""".strip()
    return """
📖 Полная карта отношений

Выбирайте этот разбор, если одного эмоционального ключа уже мало и вы хотите увидеть связную картину: как каждый чувствует близость, распознаёт симпатию, воспринимает разговор, проявляет инициативу и представляет развитие отношений.

Что внутри:
• Луна — эмоциональный ритм и условия восстановления каждого;
• Венера — как каждый может распознавать симпатию, нежность и заботу;
• Меркурий — в каком формате вам может быть легче понимать друг друга;
• Марс — как каждый проявляет желание, инициативу и реагирует на препятствия;
• Юпитер — какие смыслы и горизонты могут поддерживать развитие;
• сильные стороны, уязвимые места и небольшие эксперименты для реальной жизни.

Бесплатный мост показывает базовый эмоциональный ритм. Полная карта соединяет пять уровней и помогает выбрать, какой разговор или шаг сейчас важнее.

Это не прогноз будущего и не вердикт отношениям. Ваши решения остаются вашими.

Карта сохранится в этом разборе, и к ней можно будет вернуться через историю.
...
def premium_keyboard(product_key: str, report_id: int = 0) -> InlineKeyboardMarkup:
    ...
    if product_key == "message":
        buy_label = f"✍️ Открыть 2 варианта за {price}"
        secondary_label = "📖 Сравнить с полной картой"
        secondary_callback = "p:full"
    else:
        buy_label = f"📖 Открыть полную карту за {price}"
        secondary_label = "🪐 Выбрать одну тему за 50 ₽"
...
async def start_self(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if self_name and self_birth_date:
        ...
        await _tracked_reply_text(
            update,
            context,
            "Теперь добавим вас, чтобы увидеть не только его инструкцию, но и мост между вами. Как вас назвать в разборе? Например: я, Анна, любимая.",
            reply_markup=cancel_keyboard(),
        )
...
Это нужно, чтобы сравнить ритмы пары и предложить шаг, который учитывает вас обоих.""",
        reply_markup=cancel_keyboard(),
    )
    return ASK_WOMAN_DATE
