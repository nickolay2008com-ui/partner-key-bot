...
JUPITER_MEANINGS: dict[str, str] = {
    ...
    "water": "Рост включается через чувство, доверие, интуицию, заботу и эмоциональную безопасность. В паре это про атмосферу, где можно раскрыться глубже без давления и стыда.",
}


JUPITER_SIGN_DETAILS: dict[str, str] = {
    "aries": "Юпитер в Овне растёт через смелость, самостоятельность и право начать первым. Его вдохновляет не давление, а доверие к его импульсу.",
...
Гармония здесь не в том, чтобы кто-то стал удобнее. Она в том, чтобы превратить «почему он/я так реагирую?» в понятный мост: что даёт ему спокойствие, что даёт вам тепло и какой маленький шаг можно сделать без давления.
""".strip()
